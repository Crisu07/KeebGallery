// Imports for all Keeb Pics
import yeti5 from './keebs/yeti5.jpg'
import yeti6 from './keebs/yeti6.jpg'
import yeti4 from './keebs/yeti4.jpg'

import eighty1 from './keebs/eighty1.jpg'
import eighty2 from './keebs/eighty2.jpg'
import eighty3 from './keebs/eighty3.jpg'

import abasic1 from './keebs/abasic1.jpg'
import abasic2 from './keebs/abasic2.jpg'
import abasic3 from './keebs/abasic3.jpg'

import tiger1 from './keebs/tiger1.jpg'
import tiger2 from './keebs/tiger2.jpg'
import tiger3 from './keebs/tiger3.jpg'

import zoomr1 from './keebs/zoomr1.jpg'
import zoomr2 from './keebs/zoomr2.jpg'
import zoomr3 from './keebs/zoomr3.jpg'

import zoom1 from './keebs/zoom1.jpg'
import zoom2 from './keebs/zoom2.jpg'
import zoom3 from './keebs/zoom3.jpg'

import xov1 from './keebs/xov1.jpg'
import xov2 from './keebs/xov2.jpg'
import xov3 from './keebs/xov4.jpg'

import sonnet1 from './keebs/sonnet1.jpg'
import sonnet2 from './keebs/sonnet2.jpg'
import sonnet3 from './keebs/sonnet3.jpg'

import sun1 from './keebs/sun1.jpg'
import sun2 from './keebs/sun2.jpg'
import sun3 from './keebs/sun3.jpg'

import jjk1 from './keebs/jjk1.jpg'
import jjk2 from './keebs/jjk2.jpg'
import jjk3 from './keebs/jjk3.jpg'

import bake1 from './keebs/bakeneko1.jpg'
import bake2 from './keebs/bakeneko2.jpg'

import scarlet1 from './keebs/scarlet1.jpg'

import nk1 from './keebs/nk1.jpg'
import nk2 from './keebs/nk2.jpg'
import nk3 from './keebs/nk3.jpg'

import tiger4 from './keebs/tiger4.jpg'
import tiger5 from './keebs/tiger5.jpg'
import tiger6 from './keebs/tiger6.jpg'

import atlas1 from './keebs/atlas1.jpg'
import atlas2 from './keebs/atlas2.jpg'
import atlas3 from './keebs/atlas3.jpg'

import akko1 from './keebs/akko1.jpg'
import akko2 from './keebs/akko2.jpg'
import akko3 from './keebs/akko3.jpg'

import coarse1 from './keebs/coarse1.jpg'
import coarse2 from './keebs/coarse2.jpg'
import coarse3 from './keebs/coarse3.jpg'

const images = [
    yeti5, yeti4, yeti6,
    eighty1, eighty2, eighty3,
    abasic3, abasic1, abasic2,
    tiger3, tiger2, tiger1,
    zoomr3, zoomr1, zoomr2,
    zoom1, zoom3, zoom2,
    xov2, xov3, xov1,
    sonnet1, sonnet2, sonnet3,
    sun1, sun2, sun3,
    tiger4, tiger6, tiger5,
    atlas3, atlas1, atlas2,
    jjk1, jjk2, jjk3,
    bake1, bake2,
    scarlet1,
    nk3, nk1, nk2,
    akko1, akko3, akko2,
    coarse1, coarse2, coarse3,
  ];

  // Reference screen names for commissions to keep things a bit anounymous ATM
  const title = [
    'Persona 5', // Personal
    'Mai Sakurajima', // Personal
    'Bocchi the Rock Kikuri', // MusicallyPrime Commission
    'Cubone', // MusicallyPrime Commission
    'Hololive Houshou Marine', // MusicallyPrime Commission
    'Bocchi the Rock', // Mare Commission
    'Yueisad Commission', // Yueisad Commission
    'Rayquaza', // YoungGaryOak Commission
    'Pikachu', // Mare Commission
    'Totodile', // Katsu Commission
    'Mimikyu', // Mare Commission
    'Jujutsu Kaisen', // Personal
    'Rowlet', // Raddcatt Commission
    'Polar', // Mikurino Commission
    'Just Blue', // Derpvid Commission
    'Gengar', // Magtan1 Commission
    'ZeroTwo', // Rayven Commission
  ];
  
  // board, switches, keycaps, artisans
  // Sound tests on youtube specify more details on the board
  // prob do not need to mention what lube and film for a photo here
  const board = [
    'Axol Studios Yeti \nDurock T1 Tactile Switches lubed and filmed \nGMK Metaverse R2 Keycap Set \nS-Craft Studio Psyduck Artisan \nDykeycap Mew Artisan \nShiba Artian',

    'Mode Eighty by Mode Designs \nEMT Emilia v2 Switches stock \nKenoKeys Bunny Girl Keycap Set \nS-Craft Studio Chikorita Artisan \nBalloondog Keycaps Ditto Gengar \nand Annihilape Artisan \nokeydokey Studio Taro Bat Artisan',

    'AVA Alice Polycarb by Sneakbox \nBetty Switches lubed \nGMK Fox with EPBT Soju Novelties \nEPBT Soju Thok Bottle Cap Novelty \nKikuri Decal',

    'KBDFans Tiger80 Clear \nVermillion Bird Switches lubed \nNovelKeys Cherry Sand Keycaps \nDykeycap Cubone Artisa \nS-Craft Studio Dragonite Artisan',
    
    'Zoom75 Scarlet with OLED screen \nDreamland Tactile Switches lubed \nOutva Cthulhu Keycaps \nDykeycap Arcanine Artisan',

    'Zoom75 Pink with OLED screen \nGateron Baby Raccoon Switches stock \nJTK Night Sakura Keycaps \nS-Craft Studio Chikorita Artisan',

    'Monstargear XO v3 White \nSP Star Magic Girl Switches lubed and filmed \nGMK Godspeed Keycaps \nS-Craft Studio Umbreon and Great Ball Artisans',

    'Mode Sonnet by Mode Designs \nFlower Shadow Switches lubed and filmed \nGMK Dracula R1 Keycaps \nCustom Froakie Artisan \nDykeycap Rayquaza Artisan key and space bar \nSugico (on Etsy) Rayquaza Decal',

    'Sun68 White Gold \nSP Star Magic Girls Switches lubed and filmed \nGMK Serika Keycaps \nDykeycap Togepi Artisan \nokeydokey Studio Tai Tea Bat Artisan',

    'KDBFans Tiger80 White \nVermillion Bird Switches stock \nCannonkeys Soy Milk Keycaps \nS-Craft Studio Totodile Artisan',

    'Pantheon Lab Atlas65 \nC³Equalz X TKC Dragon Fruit Switches lubed and filmed \n Cannonkeys Esper Keycaps \nBalloondog Keycaps Gengar Ditto Artisan \nS-Craft Studio Mimikyu Artisan \nShiba Artisan',

    'Novelkeys NK65 Silver Aluminum Edition \nCannonkeys Lavender Switches lubed and filmed \nHiground Jujutsu Kaisen Keycaps \nCustom Gojo Decal',

    'Bakeneko65 NG Sage \nCannonkeys Anubis Switches lubed \nCannonkeys Bakeneko65 Keycaps \nRowlet Knife Decal (on Etsy)',

    'Scarlet TKL White \nTheKeyCo Banana Split Switches stock \nGMK Iceberg Keycap Clone \nJellyfish Artisan',

    'Novelkeys NK87 Comet Blue Aluminum Edition \nSP Star Magic Girls switches lubed \nCannonCaps Winter Tundra \nCustom Cleffairy, Togepi, and Goomy Artisans',

    'Akko SPR67 Lavender \nAkko Crystal Blue Switches lubed \nGradient Purple Keycaps \nGengar Artisan \nGengar Decal',

    'KeebsForAll Coarse60 White \nSP Star Magic Girls Switches lubed and filmed \nPBT Darling Keycaps \nZeroTwo Custom Decals',
  ];

export default images;
export {title, board};